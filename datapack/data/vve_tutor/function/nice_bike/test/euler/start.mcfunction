#vve_tutor:nice_bike/test/euler/start
# 需要安装math3.1_gelib
...
# 生成测试程序实体
data modify storage vve_tutor:io input set from storage vve_tutor:class nice_bike_plate
function vve_tutor:nice_bike/_proj
...
execute at @e[tag=math_marker,limit=1] run function vve_tutor:nice_bike/_new
tag @e[tag=result,limit=1] add test
tag @e[tag=result,limit=1] remove vve_tutor_nice_bike
scoreboard players set @e[tag=result,limit=1] target_phi 0
scoreboard players set @e[tag=result,limit=1] target_psi 0

execute as @e[tag=result,limit=1] run function marker_control:data/_get

# 设置测试程序主函数和销毁函数
data modify storage marker_control:io result.tick_func set value "vve_tutor:nice_bike/test/euler/main"
data modify storage marker_control:io result.del_func set value "vve_tutor:nice_bike/test/euler/end"
